...
class Users(Base, BaseMixin):
    ...
    upbit_access_key = Column(String(length=50), nullable=True)
    upbit_secret_key = Column(String(length=50), nullable=True)
    keys = relationship("ApiKeys", back_populates="users")
    tickers = relationship("Tickers", back_populates="users")
...
class Tickers(Base, BaseMixin):
    __tablename__ = "tickers"
    ticker = Column(String(length=10), nullable=False)
    plan = Column(Enum("rsi"), default="rsi")
    initial = Column(String(length=20), nullable=True)
    current = Column(String(length=20), nullable=True)
    rate = Column(String(length=20), nullable=True)
    expired = Column(Boolean, default=False)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    users = relationship("Users", back_populates="tickers")
